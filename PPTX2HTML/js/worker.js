...
function processPicNode(node, warpObj) {
	...
	var imgName = '';
	if(warpObj["slideResObj"][rid]!=undefined ){
		imgName = warpObj["slideResObj"][rid]["target"];
	}
    var imgFileExt = extractFileExtension(imgName).toLowerCase();
    var zip = warpObj["zip"];
	
    var imgArrayBuffer = null;
	if(imgName!=''){
		imgArrayBuffer = zip.file(imgName).asArrayBuffer();
	}
    var mimeType = "";
...
function genTextBody(textBodyNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj) {
    ...
    var text = "";
    var slideMasterTextStyles = null;
	
	if(warpObj!=undefined){
		slideMasterTextStyles=warpObj["slideMasterTextStyles"];
	}
...
function getHorizontalAlign(node, slideLayoutSpNode, slideMasterSpNode, type, slideMasterTextStyles) {
    ...
    if (algn === undefined) {
        ...
        if (algn === undefined) {
            ...
            if (algn === undefined  && (slideMasterTextStyles!=undefined || slideMasterTextStyles!=null)) {
                switch (type) {
                    case "title":
                    case "subTitle":
                    case "ctrTitle":
...
function getFontSize(node, slideLayoutSpNode, slideMasterSpNode, type, slideMasterTextStyles) {
    ...
    if (isNaN(fontSize) || fontSize === undefined && warpObj!=undefined) {
        if (type == "title" || type == "subTitle" || type == "ctrTitle") {
